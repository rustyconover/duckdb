...
struct Spinner {
	...
	duckdb::unique_ptr<duckdb::thread> spin_thread;
	string label;
	string color;
	//! Optional pointer to ShellContext for checking interrupts on Escape
	duckdb::ShellContext *interrupt_context = nullptr;
	...
	void Start(const string &new_label, const string &new_color = "\x1b[1;35m") {
		...
		spin_thread = make_uniq<duckdb::thread>([this]() {
			...
			while (running) {
				...
				if (interrupt_context && duckdb::Terminal::HasMoreData(STDIN_FILENO, 0)) {
					char c;
...
static bool EnsureHTTPFS(duckdb::ShellContext &context, AIConversationState &conv) {
	if (conv.httpfs_loaded) {
		return true;
	}
	// Try loading httpfs (may already be loaded)
	auto result = context.GetConnection().Query("LOAD httpfs");
	if (result->HasError()) {
		// Try installing first
		auto install_result = context.GetConnection().Query("INSTALL httpfs");
		if (install_result->HasError()) {
			context.PrintError(StringUtil::Format("Error: httpfs extension required for .ask but could not be loaded: %s\n",
			             install_result->GetError().c_str()));
			return false;
		}
		result = context.GetConnection().Query("LOAD httpfs");
		if (result->HasError()) {
			context.PrintError(StringUtil::Format("Error: failed to load httpfs extension: %s\n",
...
static string BuildSystemPrompt(duckdb::ShellContext &context) {
	string prompt;
	prompt.reserve(4096);
	...
	string catalog_info;
	{
		auto result = context.GetConnection().Query(
...
static string ExecuteRunSQLInternal(duckdb::ShellContext &context, AIConversationState &conv, const string &input_json) {
	...
	bool was_enabled = context.IsHighlightingEnabled();
	context.SetHighlightingEnabled(true);
	context.HighlightSQL(highlighted_sql);
	context.SetHighlightingEnabled(was_enabled);
	context.Print("\n\x1b[1;36mRunning SQL:\x1b[0m\n");
	context.Print(StringUtil::Format("%s\n\n", highlighted_sql.c_str()));

	// Execute using the shell's full execution path (supports progress bar + rendering)
	auto statements = context.GetConnection().ExtractStatements(sql);
	if (statements.empty()) {
	...
	auto result = context.GetConnection().SendQuery(std::move(statements[0]), params);

	if (result->HasError()) {
		string err = result->GetError();
		context.PrintError(StringUtil::Format("\x1b[31mError: %s\x1b[0m\n", err.c_str()));
	...
	if (result->properties.return_type == duckdb::StatementReturnType::QUERY_RESULT) {
		context.RenderQueryResult(*result);
...
static string ExecuteRunSQL(duckdb::ShellContext &context, AIConversationState &conv, const string &input_json) {
	try {
		return ExecuteRunSQLInternal(context, conv, input_json);
...
static string ExecuteListTables(duckdb::ShellContext &context) {
	auto result = context.GetConnection().Query(
	    "SELECT database_name, schema_name, table_name, table_type, "
	    "COALESCE(comment, '') as comment "
	    "FROM information_schema.tables "
...
static string ExecuteDescribeTable(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "{\"error\": \"Invalid tool input\"}";
	}
	...
	auto prepared = context.GetConnection().Prepare(query);
...
static string ExecuteAskUser(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "Error: Invalid tool input";
	}
	...
	// Read user input
	string user_input;
	user_input = context.ReadLine("\x1b[1;36mSelect\x1b[0m > ");
	if (user_input.empty() && context.ReadLineEOF()) {
		// EOF
	}
...
static string ConfirmAction(duckdb::ShellContext &context, const string &action_description) {
	context.Print(StringUtil::Format("\n%s%s%s\n", ANSI_BOLD, action_description.c_str(), ANSI_BOLD_OFF));

	string user_input;
	user_input = context.ReadLine("\x1b[1;33mAllow?\x1b[0m [Y/n/feedback] > ");
	if (user_input.empty() && context.ReadLineEOF()) {
		// EOF — treat as deny
		return "User denied this action (EOF).";
	}
	StringUtil::Trim(user_input);
...
static string ExecuteWriteFile(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "{\"error\": \"Invalid tool input\"}";
	}
	...
	while (pos <= content.size()) {
		...
		context.Print(StringUtil::Format("  %4d │ %s\n", static_cast<int>(line_num), line.c_str()));
		line_num++;
		pos = eol + 1;
		if (eol == content.size()) {
			break;
	...
	context.Print("\x1b[0m");

	auto write_denied = ConfirmAction(context, "Write this file?");
	...
	context.Print(StringUtil::Format("\x1b[32mWrote %zu bytes to %s\x1b[0m\n", written, path.c_str()));
	return "{\"ok\": true, \"message\": \"File written successfully\", \"path\": \"" + JSONEscapeString(path) +
	       "\", \"bytes\": " + to_string(written) + "}";
}

static string ExecuteRunCommand(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "{\"error\": \"Invalid tool input\"}";
	}
	...
	auto cmd_denied = ConfirmAction(context, "Run this command?");
	if (!cmd_denied.empty()) {
	...
	if (exit_code != 0) {
		context.Print(StringUtil::Format("\x1b[31mCommand exited with code %d\x1b[0m\n", exit_code));
	} else {
		context.Print(StringUtil::Format("\x1b[32mCommand completed successfully\x1b[0m\n"));
	}
...
static string ExecuteReadFile(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "{\"error\": \"Invalid tool input\"}";
	}
	...
	context.Print(StringUtil::Format("\x1b[2mRead %zu bytes\x1b[0m\n", content.size()));
...
static string ExecuteEditFile(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "{\"error\": \"Invalid tool input\"}";
	}
	...
	context.Print(StringUtil::Format("\x1b[31m- %zu chars removed\x1b[0m\n", old_str.size()));
	context.Print(StringUtil::Format("\x1b[32m+ %zu chars added\x1b[0m\n", new_str.size()));

	auto edit_denied = ConfirmAction(context, "Apply this edit?");
	...
	context.Print(StringUtil::Format("\x1b[32mEdited %s\x1b[0m\n", path.c_str()));
	return "{\"ok\": true, \"message\": \"Edit applied successfully\"}";
}

static string ExecuteGlob(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "{\"error\": \"Invalid tool input\"}";
	}
	...
	context.Print(StringUtil::Format("\x1b[2mFound %zu files\x1b[0m\n", files.size()));
...
static string ExecuteGrep(duckdb::ShellContext &context, const string &input_json) {
	auto *doc = yyjson_read(input_json.c_str(), input_json.size(), 0);
	if (!doc) {
		return "{\"error\": \"Invalid tool input\"}";
	}
	...
	context.Print(StringUtil::Format("\x1b[2mFound %d matches\x1b[0m\n", match_count));
...
static string ExecuteTool(duckdb::ShellContext &context, AIConversationState &conv, const string &tool_name,
                          const string &input_json) {
	if (tool_name == "run_sql") {
		return ExecuteRunSQL(context, conv, input_json);
	}
	if (tool_name == "list_tables") {
		return ExecuteListTables(context);
	}
	if (tool_name == "describe_table") {
		return ExecuteDescribeTable(context, input_json);
	}
	if (tool_name == "ask_user") {
		return ExecuteAskUser(context, input_json);
	}
	if (tool_name == "write_file") {
		return ExecuteWriteFile(context, input_json);
	}
	if (tool_name == "bash") {
		return ExecuteRunCommand(context, input_json);
	}
	if (tool_name == "read_file") {
		return ExecuteReadFile(context, input_json);
	}
	if (tool_name == "edit_file") {
		return ExecuteEditFile(context, input_json);
	}
	if (tool_name == "glob") {
		return ExecuteGlob(context, input_json);
	}
	if (tool_name == "grep") {
		return ExecuteGrep(context, input_json);
	}
...
static duckdb::unique_ptr<HTTPResponse> CallClaudeAPI(duckdb::ShellContext &context, const string &request_body,
                                                      const string &api_key) {
	auto &db = context.GetDatabaseInstance();
...
static void RunAgentLoop(duckdb::ShellContext &context, AIConversationState &conv, const string &api_key, const string &model,
                         const string &system_prompt) {
	Spinner spinner;
	spinner.interrupt_context = &context;
	...
	for (int round = 0; round < MAX_TOOL_ROUNDS; round++) {
		...
		auto api_thread = make_uniq<duckdb::thread>([&context, api_state]() {
			api_state->response = CallClaudeAPI(context, api_state->request_body_copy, api_state->api_key_copy);
			api_state->done = true;
		});
		...
		while (!api_state->done) {
			if (context.IsInterrupted() || !spinner.running) {
				interrupted = true;
				break;
			}
		...
		if (!response) {
			context.PrintError("Error: No response from API\n\n");
			return;
		}

		if (response->HasRequestError()) {
			context.PrintError(StringUtil::Format("Error: %s\n\n", response->GetRequestError().c_str()));
			return;
		}
		...
		if (!response->Success()) {
			...
			if (status_int == 401 || status_int == 403) {
				context.PrintError("Error: Invalid API key. Check ANTHROPIC_API_KEY.\n\n");
				return;
			}
			...
			if (status_int == 529) {
				for (int countdown = 5; countdown > 0; countdown--) {
					...
					duckdb::ThreadUtil::SleepMs(1000);
				}
				spinner.Stop();
			...
			if (!response->body.empty()) {
				...
				if (!api_resp.error_message.empty()) {
					context.PrintError(StringUtil::Format("API error (%d): %s\n\n", status_int,
					             api_resp.error_message.c_str()));
					return;
				}
			}

			context.PrintError(StringUtil::Format("API error: HTTP %d\n\n", status_int));
			return;
		}
		...
		if (!api_response.error_message.empty()) {
			context.PrintError(StringUtil::Format("API error: %s\n\n", api_response.error_message.c_str()));
			return;
		}
		...
		for (auto &block : api_response.content) {
			if (block.type == "text" && !block.text.empty()) {
				idx_t term_width = context.GetTerminalWidth();
				if (term_width == 0) {
					term_width = 100;
				}
				context.Print("\n");
				context.Print(RenderMarkdown(block.text, term_width));
			}
		}
		...
		for (auto &block : api_response.content) {
			if (block.type == "tool_use" && !block.id.empty()) {
				...
				bool is_error = false;
				try {
					result = ExecuteTool(context, conv, block.name, block.input_json);
				} catch (std::exception &e) {
	...
	context.PrintError("Error: Too many tool rounds. Try a simpler question.\n\n");
}
...
duckdb::ShellCommandResult RunAIMode(duckdb::ShellContext &context, const duckdb::vector<duckdb::string> &args) {
	...
	if (context.IsSafeMode()) {
		context.PrintError("Error: .ask is not available in safe mode\n");
		return duckdb::ShellCommandResult::FAIL;
	}

	// Initialize conversation state if needed
	auto data = context.GetExtensionData("ai_conversation");
	AIConversationState *conv_raw;
	if (!data) {
		auto conv_ptr = duckdb::make_shared_ptr<AIConversationState>();
		conv_raw = conv_ptr.get();
		context.SetExtensionData("ai_conversation", std::move(conv_ptr));
	} else {
		conv_raw = dynamic_cast<AIConversationState *>(data.get());
	}
	auto &conv = *conv_raw;
	...
	if (args.size() > 1) {
		...
		if (subcmd == "new" || subcmd == "clear") {
			...
			return duckdb::ShellCommandResult::SUCCESS;
		}
	}
	...
	if (args.size() > 1) {
		...
		string system_prompt = BuildSystemPrompt(context);
		RunAgentLoop(context, conv, api_key, model, system_prompt);
		return duckdb::ShellCommandResult::SUCCESS;
	}

	// Interactive mode
	if (conv.messages.empty()) {
		context.Print("Entering ask mode. Type /exit to return to SQL.\n\n");
	} else {
		context.Print(StringUtil::Format("Resuming conversation (%zu messages). Type /new for a fresh start.\n\n",
		             conv.messages.size()));
	}
	...
	context.SaveShellHistory(sql_history_path);
	context.ClearHistory();
	context.LoadRawHistory(ai_history_path);

	// Enter extension input mode (saves and disables SQL-specific linenoise settings)
	auto input_token = context.EnterExtensionInputMode();

	// Disable SQL syntax highlighting during AI mode
	bool saved_highlighting = context.IsHighlightingEnabled();
	context.SetHighlightingEnabled(false);
	...
	while (true) {
		...
		// Read user input
		string input = context.ReadLine("\x1b[1;36mASK\x1b[0m > ");
		if (input.empty() && context.ReadLineEOF()) {
			// Ctrl+D
			context.Print("\nExiting ask mode.\n\n");
			break;
		}
		...
		if (input == "/exit" || input == ".exit") {
			context.Print("Exiting ask mode.\n\n");
			break;
		}
		...
		if (input == "/help") {
			context.Print("/new      Start new conversation\n");
			context.Print("/model    Switch AI model\n");
			context.Print("/prompt   Show system prompt\n");
			context.Print("/exit     Return to SQL mode\n");
			context.Print("/help     Show this help\n\n");
			continue;
		}
		if (input == "/prompt") {
			context.Print(StringUtil::Format("\n%sSystem prompt (%zu chars):%s\n\n", ANSI_BOLD, system_prompt.size(), ANSI_BOLD_OFF));
			context.Print(ANSI_DIM);
			context.Print(system_prompt);
			context.Print(ANSI_RESET);
			context.Print(StringUtil::Format("\n%sConversation: %zu messages%s\n\n", ANSI_DIM, conv.messages.size(), ANSI_RESET));
			continue;
		}
		if (input == "/model") {
			context.Print(StringUtil::Format("\nCurrent model: %s%s%s\n\n", ANSI_BOLD, model.c_str(), ANSI_BOLD_OFF));
			context.Print(StringUtil::Format("  %s1.%s claude-sonnet-4-20250514 (balanced)\n", ANSI_YELLOW, ANSI_RESET));
			context.Print(StringUtil::Format("  %s2.%s claude-opus-4-20250514 (most capable)\n", ANSI_YELLOW, ANSI_RESET));
			context.Print(StringUtil::Format("  %s3.%s claude-haiku-4-5-20251001 (fastest)\n\n", ANSI_YELLOW, ANSI_RESET));
			string choice = context.ReadLine("\x1b[1;36mSelect\x1b[0m > ");
			...
			context.Print(StringUtil::Format("Model set to: %s%s%s\n\n", ANSI_BOLD, model.c_str(), ANSI_BOLD_OFF));
			continue;
		}
	...
	context.SaveHistoryToFile(ai_history_path);
	context.ClearHistory();
	context.LoadShellHistory(sql_history_path);

	// Add successful AI-executed SQL queries to the SQL history
	for (auto &sql : conv.successful_sql) {
		context.AddHistory(sql);
	}
	conv.successful_sql.clear();

	// Leave extension input mode (restores all saved linenoise settings)
	context.LeaveExtensionInputMode(input_token);

	context.SetHighlightingEnabled(saved_highlighting);
...
void RegisterAIShellCommand(duckdb::DBConfig &config) {
	auto reg = duckdb::make_shared_ptr<duckdb::ShellExtensionRegistration>();
	duckdb::ShellExtensionCommand cmd;
	cmd.command = "ask";
	cmd.argument_count = 0;
	cmd.callback = RunAIMode;
	cmd.usage = "?MESSAGE?";
	cmd.description = "Ask AI a question about your data";
	cmd.match_size = 0;
	cmd.extra_description = "Requires ANTHROPIC_API_KEY environment variable.\n"
	...
	duckdb::ShellExtensionRegistration::Register(config, std::move(reg));
}
